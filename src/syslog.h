...
#define PERR_ABORT(format,...) 		{ LOG(LOG_ERROR,LOG_PROGRAM,LOG_V0,format, ## __VA_ARGS__) ; ASSERT(false); }
#define PERRF_ABORT(fac,format,...) { LOG(LOG_ERROR,fac,LOG_V0,format, ## __VA_ARGS__) ; ASSERT(false); }
#define PERRFEX_ABORT(fac,format,...) { LOG(LOG_ERROR,fac,LOG_V0,"%s:%d " format, __FILE__,__LINE__, ## __VA_ARGS__) ; ASSERT(false); }
